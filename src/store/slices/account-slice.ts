...
export const getBalances = createAsyncThunk("account/getBalances", async ({ address, networkID, provider }: IGetBalances): Promise<IAccountBalances> => {
    ...
    const timeContract = new ethers.Contract(addresses.VERSE_ADDRESS, VerseTokenContract, provider);
    const verseBalance = await timeContract.balanceOf(address);
...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.VERSE_ADDRESS) {
        const timeContract = new ethers.Contract(addresses.VERSE_ADDRESS, VerseTokenContract, provider);
        verseBalance = await timeContract.balanceOf(address);
    ...
    if (addresses.VERSE_ADDRESS) {
        ...
        if (addresses.WSVERSE_ADDRESS) {
            sverseWsverseAllowance = await sverseContract.allowance(address, addresses.WSVERSE_ADDRESS);
        }
    }
